          ...
          <div class="middle-l" :style="middleLStyle">
            <div ref="cdWrapperRef" class="cd-wrapper">
              <div ref="cdRef" class="cd">
                <img ref="cdImageRef" class="image" :src="currentSong.pic" :class="cdClass" />
              </div>
            </div>
...
export default {
  ...
  setup () {
    ...
    const songReady = ref(false) //歌曲没有准备好 不能切换太快 会报错
    const fullScreen = computed(() => store.state.fullScreen) //!必须自己设置响应式数据 而不是之前的mapState自动到data里面就是响应式数据
    ...
    const { enter, afterEnter, afterLeave, leave, cdWrapperRef } = useAnimation()
    ...
    watch(fullScreen, async (newFullScreen) => {
      if (newFullScreen) {
        await nextTick() //!setOffset有dom相关的操作 所以我们改变视图的时候需要nextTick
    ...
    function updateTime (e) {
      ...
      currentTime.value = e.target.currentTime //有一个进度条来回跳的问题 因为我们的手指移动改变currentTime 然后这边又会自动updaterTime 同时改变currentTime 所以会来回跳
    }
    function onProgressChanging (progress) {
      progressChanging = true
      currentTime.value = progress * currentSong.value.duration
      playLyric() //歌词和currentTime同步 先偏移到我们播放的位置
      stopLyric()//停止播放 因为在拖到过程中不想他播放
    ...
    return {
      ...
      pureMusicLyric,
      playingLyric,
      onMiddleTouchStart, onMiddleTouchMove, onMiddleTouchEnd, currentShow,
...
.player {
  .normal-player {
    ...
    &.normal-leave-to {
      opacity: 0; //背景隐藏
      .top {
        transform: translate3d(0, -100px, 0); //title上移动
      }
      .bottom {
        transform: translate3d(0, 100px, 0); //icon下移动
      }
    }
  }
}
