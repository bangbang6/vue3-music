  ...
  <teleport to="body">
    ...
    <transition name="list-fade">
      <div class="playlist" v-show="visible && playlist.length" @click="hide">
          ...
          <scroll ref="scrollRef" class="list-content">
            <transition-group ref="listRef" name="list" tag="ul">
              ...
              >
                ...
                <span class="delete" @click.stop="removeSong(song)" :class="{'disabled':removing}">
        ...
        <Confirm ref="confirmRef" text="是否清空播放列表？" confirm-btn-text="清空" @confirm="confirmClear"></Confirm>
      </div>
    </transition>
  </teleport>
</template>
...
export default {
  ...
  setup () {
    ...
    async function show () {
      visible.value = true
      await nextTick()
      refreshScroll() //bsscroll必须等dom渲染出来才会计算高度
      scrollToCurrent()//滚动到目标位置
    }
    ...
    function selectItem (song) {
      ...
      }) //必须找到在playlist上的index 因为现在的index是sequenceList的index 
    ...
    return {
      ...
      visible,
      hide, show,
      playlist, getCurrentIcon, sequenceList,
      scrollRef,
      refreshScroll,
      listRef,
